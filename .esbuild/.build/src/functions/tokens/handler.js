...
// src/functions/tokens/handler.ts
var handler_exports = {};
...
function validateCard(card) {
  ...
  for (var n = card.length - 1; n >= 0; n--) {
    var cDigit = card.charAt(n), nDigit = parseInt(cDigit, 10);
    if (bEven && (nDigit *= 2) > 9)
      nDigit -= 9;
    nCheck += nDigit;
    bEven = !bEven;
...
function validateBodyCard(body) {
  const { card_number, cvv, expiration_month, expiration_year, email } = body;
  if (!validateCard(String(card_number)))
    return "invalid card";
  if (!validateCCV(cvv))
    return "invalid cvv";
  if (!validateMonth(expiration_month))
    return "invalid expiration month";
  if (!validateYear(expiration_year))
...
async function tokenize(body, pk) {
  const token = nanoid();
...
// src/functions/tokens/handler.ts
var tokens = async (event) => {
  const isValid = validateBodyCard(event.body);
  if (typeof isValid === "string")
    return jsonError(isValid);
  const pk = getPK(event.headers);
  if (pk === false)
    return jsonError("invalid pk");
  const token = await tokenize(event.body, pk);
  return formatJSONResponse({
    status: "ok",
    token
  });
};
var main = middyfy(tokens);
